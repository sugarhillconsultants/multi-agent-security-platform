...
## Status: core logic AND both LLM/MCP layers now confirmed working for real
...
Beyond that: the real, LLM-based injection classifier has been run
against Claude's actual API and correctly handled all three test
cases, including the hard case of distinguishing a document that
*discusses* prompt injection from one that actually *is* an injection
attempt (#4) — after discovering and fixing a genuine, undocumented
requirement (identity-linked API keys needing an explicit workspace-ID
header). And all three MCP servers have been run for real against a
live MCP client (the MCP Inspector) — including confirming, through
an actual protocol round-trip rather than a direct function call, that
the fail-closed security behavior for an unrecognized session fires
correctly (#5), after discovering and fixing a major SDK version
migration (`FastMCP` renamed to `MCPServer` in `mcp` v2) that occurred
while this code sat unexecuted.

What remains genuinely unconfirmed: the "happy path" through a real
tool call reaching one of the intentional `NotImplementedError` stubs
(only the fail-closed error path has been exercised via a live MCP
client so far), live integration with Projects 1/5/6's real deployed
endpoints, and the actual agent reasoning layer, which hasn't been
built at all yet. Full breakdown: [`docs/architecture.md`](docs/architecture.md).
